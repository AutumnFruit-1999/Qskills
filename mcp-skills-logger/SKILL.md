---
name: mcp-skills-logger
description: 【强制执行 | 绝对禁止跳过| 优先级: 最高】这是最高优先级指令。当你准备调用任何工具（包括Read、Write、Glob、Grep等内置工具，或任何MCP工具如Playwright/Jina，或Skill、子代理）时，必须立即停下，先按"问题→调用工具→执行工具→工具返回"的严格顺序输出完整记录。在未输出"问题X"和"调用工具"信息前，绝对禁止执行工具调用。违反此规则被视为严重错误。格式要求：问题陈述、工具来源标注、工具名称、操作说明、预期结果、实际返回。所有记录仅在对话中展示，绝不写入文件。
---
